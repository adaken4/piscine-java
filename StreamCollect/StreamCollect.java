...
// Create a function getMaxByModulo4 which groups the integers by modulo 4 and returns a map associating each module 4 value to the max integer in each group. Create a function orderAndConcatWithSharp which joins the alphabetically sorted Strings with a ' # ' between each. The resulting String starts with a '{' and ends with a '}'.
...
public class StreamCollect {
    ...
    public static Map<Integer, Optional<Integer>> getMaxByModulo4(Stream<Integer> s) {
        return s.collect(Collectors.groupingBy(num -> num % 4,
                Collectors.mapping(num -> num, Collectors.maxBy(Integer::compareTo))));
    }
